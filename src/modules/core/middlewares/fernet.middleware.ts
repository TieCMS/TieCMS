  ...
  const valid = await db
    .execute(
      db
        ...
        .table("sessions")
        .select("sessionId")
        .where(db.where.and(db.where.field("sessionId").eq(sessionId), db.where.field("userId").eq(userId))),
      webClient
    )
  ...
  if (accessToken) {
    ...
    if (isUserPayload(type, payload)) {
      ...
      if (payload.expires <= Date.now()) {
        await db.execute(db.query().delete("sessions").where(db.where.eq("sessionId", payload.sessionId)), webClient);
